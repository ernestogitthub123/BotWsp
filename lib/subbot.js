...
const openTimeout = setTimeout(() => {
  if (!connectionOpened) {
    console.log(chalk.yellow(`[⚠️ SUB-BOT ${id}] El socket no abrió a tiempo. Reiniciando...`));
    try {
...
touchSocketActivity();
const code = lastDisconnect?.error?.output?.statusCode || lastDisconnect?.reason || 0;
if (connection) {
console.log(`[CONN][subbot ${id}] state=${connection} code=${code}`);
}
