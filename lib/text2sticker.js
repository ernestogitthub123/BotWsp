...
export async function textToAnimatedSticker(text, opts = {}) {
  ...
  const holdFrames = opts.holdFrames || 4;
  const finalHoldFrames = opts.finalHoldFrames || 8;
  const align = opts.align || 'center';
  ...
  if (mode === 'assemble') {
    ...
    for (let wordIndex = 0; wordIndex < words.length; wordIndex += 1) {
      for (let frameIndex = 0; frameIndex < transitionFrames; frameIndex += 1) {
        ...
        const visibleWords = words.slice(0, wordIndex).map((word) => ({ ...word }));
  ...
  if (mode === 'glitch') {
    for (let i = 0; i < holdFrames + finalHoldFrames; i += 1) {
      const t = (holdFrames + finalHoldFrames) === 1 ? 1 : i / (holdFrames + finalHoldFrames - 1);
      const primary = palette[i % palette.length];
      const secondary = palette[(i + 2) % palette.length];
  ...
  for (let i = 0; i < holdFrames; i += 1) {
    const shift = holdFrames === 1 ? 0 : i / holdFrames;
    drawBackground(ctx, width, height, bgColor);
    drawText(ctx, layout, {
      width,
      height,
...
export async function textToAttpSticker(text, opts = {}) {
  ...
  for (let i = 0; i < frames; i += 1) {
    ...
    const accentColor = palette[(i + 1) % palette.length];
    ...
    drawText(ctx, layout, {
      ...
      strokeColor: null,
      strokeWidth: 0,
      scale: 1 + Math.sin((i / palette.length) * Math.PI) * 0.03,
      align: 'center',
      valign: 'middle',
      glowColor: mainColor,
      glowBlur: 12
